...
const API_URL = configuredApiUrl || (expoHost ? 'http://' + expoHost + ':8000/api' : 'http://YOUR_LAN_IP:8000/api');
type Tab = 'Home' | 'Explore' | 'Chats' | 'Notifications' | 'Reels' | 'Profile';
type User = { id: number; username: string; display_name?: string; bio?: string; city?: string; avatar?: string; xp?: number; karma?: number; follower_count?: number; is_followed_by_me?: boolean };
...
type Post = { id: number; kind: 'POST' | 'REEL'; body: string; media?: string; location_name?: string; quest_title?: string; recommendation_reason?: string; author: User; created_at: string; like_count: number; comment_count: number; recent_comments?: CommentItem[] };
...
function SocialQuestApp() {
  ...
  useEffect(() => {
    ...
    void NavigationBar.setBackgroundColorAsync('#050916');
    void NavigationBar.setButtonStyleAsync('light');
  }, []);

  const api = async (path: string, options: any = {}, authToken = token) => {
  ...
  const loadMe = async () => setMe(await api('/people/me/'));
  ...
  const loadMessages = async (id: number) => setMessages(rows<ChatMessage>(await api('/messages/?conversation=' + id)));
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => void loadMessages(activeChat.id), 4000);
  ...
  const authenticate = async () => {
    ...
    try {
      const body = mode === 'login'
        ...
        : { username: username.trim(), password, email: email.trim(), display_name: displayName.trim() };
  ...
  const showMessageOptions = (item: ChatMessage) => {
    if (item.deleted_for_everyone) return;
    const mine = item.sender.id === me?.id;
    const options: any[] = [];
    if (mine) options.push({ text: 'Edit message', onPress: () => setChatEditor({ kind: 'message', id: item.id, value: item.body }) });
    options.push({ text: 'Delete for me', style: 'destructive', onPress: () => void deleteMessage(item.id, 'me') });
    if (mine) options.push({ text: 'Delete for everyone', style: 'destructive', onPress: () => void deleteMessage(item.id, 'everyone') });
    options.push({ text: 'Cancel', style: 'cancel' });
    Alert.alert('Message options', mine ? 'Edit it, hide it for yourself, or remove it for everyone.' : 'You can hide this message from your own view.', options);
  };
  ...
  const share = async () => {
    ...
    try {
      ...
      if (proofQuest) {
        ...
        data.append('media_type', momentMedia?.mimeType?.startsWith('video/') ? 'REEL' : 'IMAGE');
      } else {
        data.append('kind', 'POST');
        data.append('body', moment.trim());
        if (momentLocation) data.append('location_name', momentLocation);
      }
      if (momentMedia) data.append('media', { uri: momentMedia.uri, name: momentMedia.fileName || 'socialquest-photo.jpg', type: momentMedia.mimeType || 'image/jpeg' } as any);
    ...
    } catch (error: any) { Alert.alert(proofQuest ? 'Could not share quest proof' : 'Could not share moment', error.message); }
  };
  ...
  const deletePost = (post: Post) => {
    Alert.alert('Delete this post?', 'This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => void api('/posts/' + post.id + '/', { method: 'DELETE' }).then(loadFeed).catch((error: any) => Alert.alert('Could not delete post', error.message)) },
    ]);
  };
  ...
  if (!token) {
    return <View style={styles.auth}>
      <StatusBar style="light" backgroundColor="#071022" translucent={false} />
    ...
    <Modal visible={composeOpen} transparent animationType="slide" onRequestClose={() => { Keyboard.dismiss(); setComposeOpen(false); }}>
      ...
      <Pressable style={styles.overlay} onPress={() => { Keyboard.dismiss(); setComposeOpen(false); }}>
        <Pressable style={styles.sheet} onPress={event => event.stopPropagation()}>
          ...
          <TextInput style={styles.momentInput} value={moment} onChangeText={setMoment} placeholder={proofQuest ? 'What made this moment memorable?' : 'What happened today?'} multiline />
          <View style={styles.mediaActions}>
            <Pressable style={styles.mediaAction} onPress={() => void choosePhoto()}><Text style={styles.mediaActionText}>▧ Photo library</Text></Pressable>
          ...
          {momentMedia ? <View style={styles.attachment}><Image style={styles.attachmentImage} source={{ uri: momentMedia.uri }} /><View><Text style={styles.attachmentTitle}>Photo attached</Text><Pressable onPress={() => setMomentMedia(null)}><Text style={styles.removeAttachment}>Remove</Text></Pressable></View></View> : null}
          ...
          <Pressable style={styles.primary} onPress={share}><Text style={styles.primaryText}>{proofQuest ? 'Share quest proof →' : 'Share with the community →'}</Text></Pressable>
        </Pressable>
      </Pressable>
      </KeyboardAvoidingView>
    </Modal>
    <Modal visible={!!selectedUser} transparent animationType="slide" onRequestClose={() => setSelectedUser(null)}><Pressable style={styles.overlay} onPress={() => setSelectedUser(null)}><Pressable style={styles.sheet} onPress={event => event.stopPropagation()}>{selectedUser && <><View style={styles.handle} /><Avatar user={selectedUser} large /><Text style={styles.sheetTitle}>{nameOf(selectedUser)}</Text><Text style={styles.sheetCopy}>@{selectedUser.username} · {selectedUser.city || 'Everywhere'}</Text><View style={styles.userScoreLine}><Text>★ {selectedUser.karma ?? 0} karma</Text><Text>✦ {selectedUser.xp || 0} XP</Text></View><Text style={styles.sheetCopy}>{selectedUser.bio || 'Here to turn small plans into meaningful stories.'}</Text><Pressable style={styles.primary} onPress={() => void openChat(selectedUser)}><Text style={styles.primaryText}>Send a message →</Text></Pressable><Pressable style={styles.secondary} onPress={() => void shareResource('A SocialQuest profile', 'Meet ' + nameOf(selectedUser) + ' on SocialQuest.', '/?view=profile&profile=' + selectedUser.id)}><Text style={styles.secondaryText}>↗ Share profile</Text></Pressable></>}</Pressable></Pressable></Modal>
...
const lightStyles = StyleSheet.create({
  ...
  appBrand: { color: '#0a1020', fontSize: 21, fontWeight: '800', letterSpacing: -1.1 },
  ...
  personName: { color: '#0a1020', fontSize: 15, fontWeight: '800' },
  personMeta: { color: '#747484', fontSize: 13, marginTop: 2 },
  ...
  postFooter: { borderTopColor: '#ece9e1', borderTopWidth: 1, flexDirection: 'row', gap: 20, marginTop: 14, paddingTop: 11 },
  ...
  profileTop: { alignItems: 'center', flexDirection: 'row', gap: 13 },
  ...
  commentInput: { backgroundColor: '#fff', borderColor: '#dedbd2', borderRadius: 10, borderWidth: 1, fontSize: 16, marginTop: 10, maxHeight: 100, minHeight: 48, paddingHorizontal: 13, paddingVertical: 10, textAlignVertical: 'top' },
...
Object.assign(darkStyles, {
  ...
  appBrand: { color: '#f8f6ff', fontSize: 21, fontWeight: '800', letterSpacing: -1.1 },
  ...
  personName: { color: '#f8f6ff', fontSize: 15, fontWeight: '800' }, personMeta: { color: '#a2a5b6', fontSize: 13, marginTop: 2 },
  recommendationTag: { alignSelf: 'flex-start', backgroundColor: '#20243b', borderRadius: 12, color: '#7de5c3', fontSize: 11, fontWeight: '800', marginTop: 11, paddingHorizontal: 8, paddingVertical: 5 }, questPostTag: { alignSelf: 'flex-start', backgroundColor: '#302647', borderRadius: 12, color: '#caa5ff', fontSize: 11, fontWeight: '800', marginTop: 11, paddingHorizontal: 8, paddingVertical: 5 }, postBody: { color: '#e9e8f1', fontSize: 17, lineHeight: 25, marginTop: 13 }, postFooter: { borderTopColor: '#2d3040', borderTopWidth: 1, flexDirection: 'row', gap: 20, marginTop: 14, paddingTop: 11 }, postManage: { borderLeftColor: '#343746', borderLeftWidth: 1, flexDirection: 'row', gap: 10, marginLeft: 'auto', paddingLeft: 10 }, postManageText: { color: '#cfcbdf', fontSize: 13, fontWeight: '800' }, postDeleteText: { color: '#ff74aa', fontSize: 13, fontWeight: '800' },
...
Object.assign(darkStyles, {
  ...
  appBrand: { color: '#f8f5e8', fontFamily: PIXEL_FONT, fontSize: 13, letterSpacing: -1 },
  ...
  personName: { color: '#f8f5e8', fontFamily: 'monospace', fontSize: 15, fontWeight: '800' },
  personMeta: { color: '#aeb9d1', fontFamily: 'monospace', fontSize: 12, marginTop: 3 },
  ...
  postFooter: { borderTopColor: '#263b62', borderTopWidth: 1, flexDirection: 'row', gap: 15, marginTop: 13, paddingTop: 10 },
  ...
  commentInput: { backgroundColor: '#071022', borderColor: '#536c9d', borderRadius: 1, borderWidth: 1, color: '#f8f5e8', fontFamily: 'monospace', fontSize: 16, marginTop: 10, maxHeight: 100, minHeight: 48, paddingHorizontal: 12, paddingVertical: 10, textAlignVertical: 'top' },
...
Object.assign(darkStyles, {
  ...
  appBrand: { ...darkStyles.appBrand, fontSize: 14 },
  ...
  navLabel: { ...darkStyles.navLabel, fontSize: 8 },
  eventName: { ...darkStyles.eventName, fontSize: 17 },
  personName: { ...darkStyles.personName, fontSize: 16 },
